...
public class ClawPlayerController : MonoBehaviour {
	...
	void Update () {
		...
		if (!speedSwitch && myRB.velocity.magnitude < myNavAgent.speed) {								//makes sure plyer doesnt walk back when hit
		...
		if (Input.GetKeyDown (KeyCode.S)) {						//makes the player back stack when you hit the s key
		...
		if (Input.GetButtonDown ("Fire1") && !dug) {															// hit leftClick, make ray for player to follow
		...
		if(Input.GetKeyDown (KeyCode.Space)){		//dig in ground code

			if (dug){		//if dug into the ground, come out of ground
				dug=false;
				Destroy (dirt);
			...
			}else{		//if not dug in, start digging
				Destroy(markerClone);
				Destroy (preMarkerClone);
			dug=true;
